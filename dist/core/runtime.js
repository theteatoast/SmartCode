...
export class SmartCodeRuntime {
    ...
    constructor(config) {
        ...
        this.loopDetector = new LoopDetector(config.optimizationLevel);
        this.analytics = new Analytics(this.sessionId, config.name, config.optimizationLevel, config.responseStyle);
    ...
    async runSession() {
        ...
        this.sessionManager.createSession(this.sessionId, this.config.name, this.config.optimizationLevel, this.config.responseStyle);
        // --- Wire up event handlers ---
        // Track agent output for analytics
        this.ptyManager.on('data', (event) => {
            this.analytics.recordAgentOutput(event.raw.length);
        });
        // Analyze complete agent turns for loop patterns
        this.ptyManager.on('agent-turn', (output) => {
        ...
        this.ptyManager.on('user-input', (input) => {
